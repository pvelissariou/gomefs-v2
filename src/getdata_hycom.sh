...
pushd ${DataDir} >/dev/null
  GPAR_JOBLOG="${LogDir}/${theSCRIPT%%.*}-status.log"
  GPAR_RUNLOG="${LogDir}/${theSCRIPT%%.*}-run.log"

  GPAR_OPTS="${GPAR_OPTS_GLB} ${GPAR_OPTS_SSH} ${GPAR_OPTS_TIME}"
  GPAR_OPTS="${GPAR_OPTS} --joblog ${GPAR_JOBLOG} ${GPAR_OPTS_RESUME}"
  GPAR_OPTS="${GPAR_OPTS} --wd ${DataDir} -j0"
  ...
  ${GPAR} ${GPAR_OPTS} --xapply ${theSCRIPT} {1} {2} \
    ::: $(cat ${theINPLIST}) \
    ::: $(cat ${theOUTLIST}) >> ${GPAR_RUNLOG} 2>&1
  FAILURE_STATUS=$?
