...
#      setenv W     /u/home/metzger/force/climo_extend/nogaps0.5a/wndspd/
#      setenv W      /p/cwfs/dsfrank/force/climo_extend/nogaps0.5a/wndspd/
#      setenv W      /p/cwfs/dsfrank/force/climo_extend/navgem0.5a/wndspd/
      setenv W      /scr/ooc/data/hycom/preproc/force/navgem/1.2_0.5a/3hrly/climo_extend/wndspd
    else
#                  Single Disk
      setenv S ~/$P/SCRATCH
      setenv W ~/temp_ieee/$N
...
#  touch  fort.${N}
#  if (-z fort.${N}) then
#      ${pget} ${W2}/3hourly/navgem0.5a-sec_${y}_03hr_wndspd.D fort.${N} &
#  endif
#end
#
# real time
...
touch  fort.71 fort.72
if (-z fort.71) then
#  ${pget} ${W}/nogaps0.5a_wndspd.D fort.71 &
  /bin/cp ${W}/navgem0.5a_wndspd.D fort.71 &
endif
if (-z fort.72) then
#  ${pget} ${W}/nogaps0.5a-sec_clim_wndspd.D fort.72 &
  /bin/cp ${W}/navgem0.5a-sec_clim_wndspd.D fort.72 &
endif
C
C --- executable
C
...
cat <<E-o-D  > fort.05in
 &AFTITL
  ...
  CTITLE = 'NAVGEM 0.5a, 3hrly, ocn-only, scat scaled',
  CNAME  = 'wndspd',
 &END
 &AFTIME
  FSTART = ${TS},
...
C --- Output.
C
/bin/mv fort.10  wndspd_${idtg}.b
/bin/mv fort.10a wndspd_${idtg}.a
#
#set sizefl=`ls -la wndspd_${idtg}.a | awk '{print $5}'`
#echo ${E} GOMl0.04 ${sizefl} >! ~/${P}/wrongforcing.txt
#if ( ${sizefl} != '83312640' ) then
# mail -s 'SMALL WNDSPD FILE ' ${user} < ~/${P}/wrongforcing.txt
#endif
C
if (-e ./SAVE) then
