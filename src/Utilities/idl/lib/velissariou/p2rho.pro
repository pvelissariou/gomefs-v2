...
;
; PURPOSE:
;	Interpolate from field at psi points to a field at rho points.
;
; AUTHOR:
;
...
;
; CALLING SEQUENCE:
;	  outData = P2Rho(data [, mask] [, keywords])
;	     data - The 2D/3D array of the variable values at psi points
...
;
; RETURNS:
;         outData - The 2D/3D array of the variable calculated at RHO
...
;+++
FUNCTION P2Rho, data, mask, MASK_VAL = mask_val
  ...
  if (n_elements(mask) ne 0) then begin
    ...
    do_mask = 1
    mask_dat = UVP_Mask(mask, /PLOC)
    DRYIDX = where(mask le 0, DRYCNT, COMPLEMENT = WETIDX, NCOMPLEMENT = WETCNT)
  ...
  case nDIMS of
    2: $
      begin
        ...
        inpData = data
        if (do_mask gt 0) then inpData = ZeroFloatFix( inpData * mask_dat )
        inpData = UVP2Rho_Points(inpData, /PLOC)
        if( do_fill gt 0 ) then inpData[DRYIDX] = fill_val
        outData[*, *] = inpData
      end
    3: $
      begin
        outData = make_array(idim_rho, jdim_rho, kdim, $
                             TYPE = size(data, /TYPE), VALUE = 0)
