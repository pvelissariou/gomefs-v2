...
;
; PURPOSE:
;	To calculate the maximum Haney (rx1) grid stiffness parameter.
;       The rx1 calculated values represent the maximum value of rx1
...
;
; CALLING SEQUENCE:
;      rx_out = Stiff_Haney(nlay, zarr, marr, rx, $
;                           [[/report,][/eight,][MAX_RX = max_rx]])
;
;            nlay - The number of the sigma/z layers (NOT LEVELS). The size
;                   of zarr below is to be: zarr[IPNTS, JPNTS, NLAY + 1].
;	     zarr - The depth array (3D). It is assumed that the values are
;                   reported at the z-levels (negative, in sigma
;                   streched/unstreched formulation) at the gridded locations
;                   (zero at the free surface and -H at the bottom, THAT IS
;                   AT THE W-POINTS). The size of the zarr should be:
;                   Size: zarr[IPNTS, JPNTS, NLAY + 1].
...
;                   values of the Haney stiffness parameter for each layer.
;                   Size: rx[IPNTS, JPNTS, NLAY].
;          max_rx - Set this keyword to a named variable that receives the
;                   values of the maximum (over the vertical direction) of
;                   the Haney stiffness parameter.
;                   Size: max_rx[IPNTS, JPNTS].
...
;+++
  ...
  ; --------------------
  ; check the number of the layers
  If (N_Elements(nlay) EQ 0) Then Message, "Must pass the <nlay> argument."
  If (Max(Where([2, 3, 12, 13, 14, 15] EQ Size(nlay, /TYPE))) LT 0) Then $
    Message, "Only integers are valid values for <nlay>."
  ...
  If (Size(zarr, /N_DIMENSIONS) NE 3) Then $
    Message, "<zarr> must be a 3D array of values."
  ...
  If (KPNTS NE (n_lay + 1)) Then $
    Message, "The vertical dimension of <zarr> is not equal to (nlay + 1)."
  ...
  Rx     = Make_Array(IPNTS, JPNTS, n_lay, /FLOAT, VALUE = Mask_Rx)
  Max_Rx = Make_Array(IPNTS, JPNTS, /FLOAT, VALUE = Mask_Rx)
  ...
  For imsk = 0L, mskCNT - 1 Do Begin
    ...
    For k = 1L, KPNTS - 1 Do Begin
      nIJ = N_Elements(IdxArr[0, *])
      TmpRX = Make_Array(nIJ, /FLOAT, VALUE = 0.0)

      For ij = 0L, nIJ - 1 Do Begin
        val1 = zarr[i, j, k]
        val2 = zarr[IdxArr[0, ij], IdxArr[1, ij], k]
        val3 = zarr[i, j, k - 1]
        val4 = zarr[IdxArr[0, ij], IdxArr[1, ij], k - 1]

        absval1 = Abs(val1 - val2 + val3 - val4)
        absval2 = Abs(val1 + val2 - val3 - val4)

        TmpRX[ij] = (absval1 LE 0.0001) ? 0.0 : (absval1 / absval2)
      EndFor
      Rx[i, j, k - 1] = Max(TmpRX)
    EndFor
    Max_Rx[i, j] = Max(Reform(Rx[i, j, *]))
  EndFor
