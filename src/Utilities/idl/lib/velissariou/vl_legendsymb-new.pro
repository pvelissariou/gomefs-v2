...
if (nLine gt 1) then begin
  if (nLine ge nText) then begin
    thisLine[0:nText-1] = double(line[0:nText-1])
  endif else begin
    thisLine[0:nLine-1] = line[0:nLine-1]
  endelse
endif else begin
  if (nLine eq 1) then thisLine[0:nText-1] = double(line[0])
endelse
nLine = nText
...
if (nThick gt 1) then begin
  if (nThick ge nText) then begin
    thisThick[0:nText-1] = double(thick[0:nText-1])
  endif else begin
    thisThick[0:nThick-1] = thick[0:nThick-1]
  endelse
endif else begin
  if (nThick eq 1) then thisThick[0:nText-1] = double(thick[0])
endelse
nThick = nText
...
LabOffX = ch_xsize / double(!D.X_SIZE)
