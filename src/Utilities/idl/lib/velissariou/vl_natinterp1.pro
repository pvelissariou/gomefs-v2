...
;       This is faster than the VL_NatInterp procedure.
; CALLING SEQUENCE:
;	zloc = VL_NatInterp1(xloc, yloc, xdat, ydat, zdat, [Options])
;
;       Uses the sort_sd procedure from the coyote library.
;       Uses the nearestn procedure from the beaumont library.
;
;	On input:
...
;+++
FUNCTION VL_NatInterp1, xloc, yloc, xdat, ydat, zdat,   $
                        NPOINTS = npoints,              $
  ...
  np_idx = nearestn(transpose([ [thisXLOC], [thisYLOC] ] ), $
                    transpose([ [inpXDAT], [inpYDAT] ] ),   $
                    nearpnts, /all)
  ...
  for i = 0L, nLOC - 1 do begin
    ...
    ; in ascending order
    xx = thisXDAT[idxDAT] - thisXOUT
    yy = thisYDAT[idxDAT] - thisYOUT
    dd = sqrt(xx * xx + yy * yy)
    ...
    thisXDAT = thisXDAT[idxDAT[idx_sort[0:np - 1]]]
    thisYDAT = thisYDAT[idxDAT[idx_sort[0:np - 1]]]
    thisZDAT = thisZDAT[idxDAT[idx_sort[0:np - 1]]]
