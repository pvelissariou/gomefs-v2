Function XDist, lat, lon, Kilometers = kilometers
;+++
; NAME:
;	XDist
; VERSION:
;	1.0
; PURPOSE:
;	To return the x distance in meters from the grid origin
...
; RETURNS:
;	The x distance in meters from the grid origin.
; SIDE EFFECTS:
...
;
; first rotate
  dout = Double( xp * cos(alpha) + yp * sin(alpha) )
; now translate
  dout = Double( dout + iparm[2] * GridXSZ )
