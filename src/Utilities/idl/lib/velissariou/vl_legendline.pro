...
;
; CALLING SEQUENCE:
;	VL_LegendLine, Position, Label, [keywords]
;
...
;                        Default :  0.02
;       line        :   A vector, of type integer, containing the line type
;                        index value for each line.
;                        Default :  0
...
;	  tek_color
;	  VL_LegendLine, [0.2,0.2], ['Red','Green'], color=[2,3], $
;                        title='Legend'
;
...
; ----- Line types
thisLine = make_array(nText, /INTEGER, VALUE = 0)
nLine = n_elements(line)
if (nLine gt 1) then begin
  if (nLine ge nText) then begin
    thisLine[0:nText-1] = fix(abs(line[0:nText-1]))
  endif else begin
    thisLine[0:nLine-1] = fix(abs(line[0:nLine-1]))
  endelse
endif else begin
  if (nLine eq 1) then thisLine[0:nText-1] = fix(abs(line[0]))
endelse
nLine = nText
...
; ----- Line lengths
thisLength = n_elements(length) eq 0 ? 0.02 : abs(length)
...
LabOffX = 0.5 * ch_xsize / double(!D.X_SIZE) ; in normal coordinates
LegHeight = LabOffset + total(LabHeight) + total(LegLineDist) + 2.0 * yOff
LegWidth  = thisLength + LabOffX + maxLabWidth + 2.0 * xOff
...
for i = 0, nText - 1 do begin
  jj = nL0 + 2 * i
  kk = nT0 + i
  xx2 = xx1 + LabOffX + thisAlign[i] * maxLabWidth
  yy = (i eq 0) ? yy : yy + LabHeight[i - 1] + LegLineDist[i]
  LegCoord[*, jj]     = [ xx,  yy + LabDesc[i]]
  LegCoord[*, jj + 1] = [ xx1, yy + LabDesc[i]]
  LegCoord[*, kk]     = [ xx2, yy]
endfor
...
for i = 0, nText - 1 do begin
  j = nL0 + 2 * i
  xx = [LegCoord[0, j], LegCoord[0, j+1]]
  yy = [LegCoord[1, j], LegCoord[1, j+1]]
  ...
  if (abs(thisRotation) gt 0.0) then begin
    ...
    !P.T = t3dmat
    plots, xx, yy, color = thisColor[i], linestyle = thisLine[i], thick = thisThick[i], $
